...
const EventCard: React.FC<EventCardProps> = ({ event, showStudioJoin }) => {
  ...
  // Determine reward unit based on event type
  const rewardUnit = event.status === EventStatus.TESTING ? 'TP' : 'LC';
    ...
    <>
      ...
      >
          ...
          <div className="absolute top-4 left-4 flex space-x-2 z-10">
            <span className={`${getStatusColor(event.status)} text-white text-[10px] font-black px-2 py-0.5 rounded uppercase shadow-lg`}>
              {event.status === EventStatus.ONGOING ? 'ONGOING' : 
               ...
               event.status === EventStatus.TESTING ? 'TEST' : 'CLOSED'}
            </span>
            {isJoined && (
        ...
        <div className="p-6 flex-1 flex flex-col">
          <div className="mb-4">
            <h3 className="font-black text-gray-800 text-xl tracking-tight line-clamp-1 uppercase">{event.name}</h3>
          </div>
            ...
            <div className="space-y-2.5 pt-2">
              {event.stages.slice(0, 2).map((stage) => (
                <div key={stage.stage} className="flex items-center space-x-3 text-xs">
                  ...
                  <span className={`font-black shrink-0 ${stage.isCompleted ? 'text-gray-300' : 'text-yellow-600'}`}>+{stage.rewardAmount} {rewardUnit}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-8 space-y-3">
            {showStudioJoin && !isJoined && (
              <button 
                ...
                className="w-full py-3 rounded-2xl font-black text-xs transition-all flex items-center justify-center space-x-2 border-2 border-indigo-100 text-indigo-500 hover:bg-indigo-50 active:scale-95 uppercase tracking-widest"
              >
